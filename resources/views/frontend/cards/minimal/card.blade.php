<div id="card-{{$card->id}}" class="one-offer @if($card->promo == 1) spec-offer @endif @if($card->label != 0)label label{{$card->label}} @endif">
    ...
    <div class="d-flex card-mn">
        ...
        <div class="card-mn-main-details">
            @if(file_exists( base_path().'/resources/views/frontend/cards/minimal/fields/'.$card->category_id.'-head.blade.php'))
                @include("frontend.cards.minimal.fields.$card->category_id-head")
            @endif
        </div>
        <?php
    ...
    <div class="card-mn-details-block display_none">
        @if(file_exists( base_path().'/resources/views/frontend/cards/minimal/fields/'.$card->category_id.'-body.blade.php'))
            @include("frontend.cards.minimal.fields.$card->category_id-body")
        @endif
            <div class="card-mn-icons-slider-js">
                <?php
                ...
                @foreach ($current_vzo_icons as $current_item_icon)
                    @if (isset($all_vzo_icons[$current_item_icon]))
                        <div><img class="" src="/images/ic/icon-{{$current_item_icon}}.png"
                                  alt="{{$all_vzo_icons[$current_item_icon]['name']}}"><br><span>{{$all_vzo_icons[$current_item_icon]['name']}}</span>
                        </div>
                    @endif
                @endforeach
            </div>
        ...
        @else
            <div class="card-mn-icons-slider display_none">
                <?php
                $all_vzo_icons = \Config::get('icons');
                $current_vzo_icons = explode(',', $card->icons);
                ?>
