...
<div class="side-block">
    ...
    <div class="side-box">
        <ul class="mb0 rating-mfk">
            @if($bank->show_credits && Request::path() != "banki/$bank->alias/credits")
            <li><a href="/banki/{{$bank->alias}}/credits" class="fa-icon fa-bank">Кредиты</a></li>
            @endif
            @if($bank->show_credit_cards && Request::path() != "banki/$bank->alias/credit-cards")
            <li><a href="/banki/{{$bank->alias}}/credit-cards" class="fa-icon fa-credit-card">Кредитные карты</a></li>
            @endif
            @if($bank->show_debit_cards && Request::path() != "banki/$bank->alias/debit-cards")
            <li><a href="/banki/{{$bank->alias}}/debit-cards" class="fa-icon fa-credit-card-alt">Дебетовые карты</a></li>
            @endif
            @if($bank->show_auto_credits && Request::path() != "banki/$bank->alias/autocredit")
            <li><a href="/banki/{{$bank->alias}}/autocredit" class="fa-icon fa-automobile">Автокредиты</a></li>
            @endif
            @if($bank->show_deposits && Request::path() != "banki/$bank->alias/deposits")
            <li><a href="/banki/{{$bank->alias}}/deposits" class="fa-icon fa-gift">Вклады</a></li>
            @endif
            @if($bank->show_mortgage && Request::path() != "banki/$bank->alias/mortgage")
            <li><a href="/banki/{{$bank->alias}}/mortgage" class="fa-icon fa-home">Ипотеки</a></li>
            @endif
