@if(count($cards)>3)
    ...
    @else
        <h2 class="fdsb listing-h2 text-center" id="comparison">Итоговое сравнение ипотечных кредитов с онлайн-заявкой</h2>
    @endif
    <table class="total_cards_table">
        <thead>
        <tr>
            <th class="display_none"></th>
            <th>@if(isset($tag_name)) @if($tag_name!='')Ипотека {{$tag_name}} @else Название @endif @endif</th>
        ...
        @foreach($cards as $card)
            <tr>
                ...
                <td class="text-center">{{$card->header_2}} лет</td>
...
@section('additional-scripts')
    ...
    @stack('credit-additional-scripts')
