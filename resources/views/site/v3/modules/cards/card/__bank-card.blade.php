...
{{--            <span  data-id="{{$card->id}}" class="complaint"><i class="fa fa-warning"></i> <span>Подать жалобу</span></span>--}}
        ...
        <div class="col-md-8 mob-initial">
            @if(isset($card->ccid))
                <?php /*
                                <span class="card_remove btn btn-danger btn-xs" data-ccid="{{$card->ccid}}" data-id="{{$card->id}}"><i class="fa fa-remove"></i></span> */ ?>
            @endif
            <div class="top-cart">
    ...
    <?php /*
                        <div class="card_lists">
                            @if(isset($card->list_of_benefits))
                            @if($card->list_of_benefits != null)
                                <div class="card_list_inner">
                                    <span>Преимущества</span>
                                    <ul class="list_of_benefits">
                                        <?php $ul = explode(';',$card->list_of_benefits); ?>
                                        @foreach($ul as $li)
                                            <li>{{$li}}</li>
                                        @endforeach
                                    </ul>
                            ...
                            @endif @endif
                            @if(isset($card->list_of_disadvantages))
                            @if($card->list_of_disadvantages != null)
                                <div class="card_list_inner">
                                    <span>Недостатки</span>
                                    <ul class="list_of_disadvantages">
                                        <?php $ul = explode(';',$card->list_of_disadvantages); ?>
                                        @foreach($ul as $li)
                                            <li>{{$li}}</li>
                                        @endforeach
                                    </ul>
                            ...
                            @endif @endif
                            @if(isset($card->list_of_notes))
                            @if($card->list_of_notes != null)
                                <div class="card_list_inner">
                                    <span>Примечания</span>
                                    <ul class="list_of_notes">
                                        <?php $ul = explode(';',$card->list_of_notes); ?>
                                        @foreach($ul as $li)
                                            <li>{{$li}}</li>
                                        @endforeach
                                    </ul>
