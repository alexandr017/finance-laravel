...
@section('content')
    ...
    <form method="POST" action="/admin/cards/cards/edit_save" novalidate>
        ...
        <div class="tab-content">
            ...
            <div role="tabpanel" class="tab-pane" id="listings">
                <h2>Привязка на основании перелинковки</h2>
                <div class="form-group">
                    <div id="ajax_listings">
                        @foreach($relink as $groupName => $group)
                            <details class="listings-group">
                                <summary><i class="fa fa-angle-right"></i> <b>{{$groupName}}</b></summary>
                                <button class="btn btn-success btn-xs check-all">Отметить все</button>
                                <button class="btn btn-warning btn-xs uncheck-all">Снять все</button><br>
                                @foreach($group as $value)
                                        <?php
                                        $link = str_replace('https://vsezaimyonline.ru', '', $value->link);
                                        $link = preg_replace("/^\//", '', $link);
                                        ?>
                                    <div class="checkbox width-50">
                                        <label>
                                            <input name="listings[]" value="{{ $link }}" type="checkbox" @if(isset($listings[$link])) checked="true" @endif>
                                            {{$value->title}}
                                        </label>
                                    </div>
                                @endforeach
                ...
                <h2>Привязка на основании регионов</h2>
                <div class="form-group">
                    <div id="ajax_listings">
                        @foreach($regions as $groupName => $region)
                            <details class="listings-group">
                                <summary><i class="fa fa-angle-right"></i> <b>{{$groupName}}</b></summary>
                                <button class="btn btn-success btn-xs check-all">Отметить все</button>
                                <button class="btn btn-warning btn-xs uncheck-all">Снять все</button><br>
                                @foreach($region as $city)
                                    <div class="checkbox width-50"><label><input name="listings_city[]" value="{{ $city->transliteration }}" type="checkbox" @if(isset($listings[$city->transliteration])) checked="true" @endif>{{$city->imenitelny}}</label></div>
                                @endforeach
                            </details>
                        @endforeach
                    </div>
    ...
    <style type="text/css">
        ...
        details {
            cursor: pointer;padding: 15px 15px 0 15px;border-radius: 5px;border: 1px solid #ccc; margin-bottom: 5px
    ...
    <script type="text/javascript">
        ...
        document.querySelectorAll('.listings-group').forEach(listingsGroup => {
            let checkAllBtn = listingsGroup.querySelector('.check-all');
            let uncheckAllBtn = listingsGroup.querySelector('.uncheck-all');

            let checkboxes = listingsGroup.querySelectorAll('.checkbox input');
