...
class Card8 extends Model
{
    protected $table = 'cards_8';
    ...
    public static function go($request, $id,$action){
        ...
        $this2->security = (empty($request['security'])) ? null :  $request['security'];
        $this2->an_initial_fee = (empty($request['an_initial_fee'])) ? null :  $request['an_initial_fee'];
    ...
    public static function parse_fields($code,$card){
        ...
        $code = str_replace('{security}', $card->security, $code);
        $code = str_replace('{an_initial_fee}', $card->an_initial_fee, $code);
