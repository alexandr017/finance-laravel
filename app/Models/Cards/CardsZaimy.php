...
class CardsZaimy extends Model
{
    protected $table = 'cards_1_zaimy';
    ...
    public static function go($request, $id,$action){
    	...
    	$this2->poor_ch = (empty($request['poor_ch'])) ? null :  $request['poor_ch'];
    ...
    public static function ratingUpdate(){
        ...
        $companies = DB::table("companies")
            ...
            ->where(['companies.group_id'=>1,'urls.section_type'=>5,'companies.status'=>1])
        ...
        if($update){
            ...
            for($i=1; $i<6; $i++){
                ...
                $company_name = ($currentCompany->company_name != null) ? $currentCompany->company_name : $currentCompany->h1;
                $curentBDItem->company_name = $company_name;
                $last_pos = null;
                foreach ($sidebarRating as $key => $value){
                    if($value->company_name == $company_name) $last_pos = $value->id;
                }
