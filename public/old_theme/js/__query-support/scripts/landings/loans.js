$(function() {
    function sort(sort_by_field,sort_direction) {
        var nodeList = document.querySelectorAll('.beta-card-simple .beta-card-main-details');
        var itemsArray = [];
        ...
        itemsArray.sort(function(nodeA, nodeB) {
            var numberA = Number((nodeA.querySelector('.beta-card-simple .sorting-field-'+field+'').textContent).replace(/\s/g, ''));
            var numberB = Number((nodeB.querySelector('.beta-card-simple .sorting-field-'+field+'').textContent).replace(/\s/g, ''));
            if(sort_direction == 0){
